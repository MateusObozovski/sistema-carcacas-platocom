...
export default function ClientesPage() {
        ...
        const clientesComStats = await Promise.all(
          (clientesData || []).map(async (cliente) => {
            try {
              ...
              console.error(
                `[v0] Error fetching stats for client ${cliente.id}:`,
                error
              );
              return {
                ...cliente,
  ...
  const clientesFiltrados = clientes
    .filter((cliente) => {
      ...
      const matchVendedor =
        vendedorFiltro === "todos" || cliente.vendedor_id === vendedorFiltro;

      return matchSearch && matchVendedor;
    })
    .sort((a, b) => (b.debito || 0) - (a.debito || 0));

  const debitoTotal = clientesFiltrados.reduce(
    (acc, c) => acc + (c.debito || 0),
    0
  );
