...
export default function DashboardPage() {
  ...
  if (
    ...
    const carcacasPendentesTotal = vendedores.reduce(
      (acc, v) => acc + v.carcacasPendentes,
      0
    );
    const pedidosAtrasados = orders.filter(
      (p) => p.status === "Atrasado"
    ).length;
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Visão geral do sistema de carcaças
            </p>
          </div>
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  <Button
                    ...
                    asChild
                  >
                    <Link href="/carcacas-pendentes" prefetch={false}>
                      Ver Todas as Carcaças Pendentes
                    </Link>
                  </Button>
                </div>
              </CardContent>
  ...
  if (user?.role === "Vendedor") {
    ...
    const pedidosAtrasados = meusPedidos.filter(
      (p) => p.status === "Atrasado"
    ).length;
      ...
      <ProtectedRoute allowedRoles={["Vendedor"]}>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Visão geral das suas vendas e débitos
            </p>
          </div>
                  ...
                  <TableBody>
                    {meusClientes.length === 0 ? (
                      ...
                      meusClientes.map((cliente) => {
                        ...
                        const clientePedidos = meusPedidos.filter(
                ...
                <Table>
                  ...
                  <TableBody>
                    {pedidosPendentes.slice(0, 5).map((pedido) => {
                      const cliente = meusClientes.find(
                        (c) => c.id === pedido.cliente_id
                      );
                      return (
                        <TableRow key={pedido.id}>
