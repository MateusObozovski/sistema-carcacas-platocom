...
export default function RelatoriosPage() {
  ...
  const pedidosFiltrados = orders.filter((p) => {
    ...
    if (vendedorFiltro !== "todos" && p.vendedor_id !== vendedorFiltro)
      return false;
  ...
  const vendasBaseTroca = pedidosFiltrados.filter(
    (p) => p.tipo_venda === "Base de Troca"
  ).length;
  const vendasNormais = pedidosFiltrados.filter(
    (p) => p.tipo_venda === "Normal"
  ).length;
  const valorTotalVendas = pedidosFiltrados.reduce(
    (acc, p) => acc + (p.valor_total || 0),
    0
  );
  const debitoTotal = carcacasPendentes.reduce(
    (acc, p) => acc + calcularDebitoPendente(p),
    0
  );
  const pedidosAtrasados = carcacasPendentes.filter(
    (p) => p.status === "Atrasado"
  ).length;

  const vendedoresComDados = vendedores.map((vendedor) => {
    const pedidosVendedor = pedidosFiltrados.filter(
      (p) => p.vendedor_id === vendedor.id
    );
    const carcacasVendedor = carcacasPendentes.filter(
      (p) => p.vendedor_id === vendedor.id
    );
    const debitoVendedor = carcacasVendedor.reduce(
      (acc, p) => acc + calcularDebitoPendente(p),
      0
    );
    ...
    return {
      ...
      valorVendas: pedidosVendedor.reduce(
        (acc, p) => acc + (p.valor_total || 0),
        0
      ),
  ...
  const clientesComDados = clients.map((cliente) => {
    const pedidosCliente = pedidosFiltrados.filter(
      (p) => p.cliente_id === cliente.id
    );
    const carcacasCliente = carcacasPendentes.filter(
      (p) => p.cliente_id === cliente.id
    );
    const debitoCliente = carcacasCliente.reduce(
      (acc, p) => acc + calcularDebitoPendente(p),
      0
    );
    // Contar itens pendentes, não pedidos
    ...
    return {
      ...
      valorVendas: pedidosCliente.reduce(
        (acc, p) => acc + (p.valor_total || 0),
        0
      ),
      carcacasPendentesAtual: itensPendentes,
  ...
  const handleExportarPDF = async (
    tipo: "vendas" | "carcacas" | "pedidos-pendentes"
  ) => {
    try {
      const {
        generateVendasCompletoPDF,
        generateCarcacasPendentesPDF,
        generatePedidosPendentesPDF,
      } = await import("@/lib/pdf-reports");
      ...
      switch (tipo) {
        case "vendas":
          ...
          break;
        case "carcacas":
          await generateCarcacasPendentesPDF(
            orders,
            filters,
            clients,
            vendedores
        ...
        case "pedidos-pendentes":
          await generatePedidosPendentesPDF(
            orders,
            filters,
            clients,
            vendedores
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Selecione o período e vendedor para análise
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="flex justify-end">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => handleExportarPDF("pedidos-pendentes")}
              >
                <FileText className="mr-2 h-4 w-4" />
        ...
        <Tabs defaultValue="desempenho-mensal" className="space-y-4">
          <TabsList>
            <TabsTrigger value="desempenho-mensal">
              Desempenho Mensal
            </TabsTrigger>
          ...
          <TabsContent value="desempenho-mensal" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Análise de pedidos e carcaças devolvidas mês a mês
                </CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  // Agrupar pedidos por mês/ano e vendedor
                  const desempenhoMensal: Record<
                    string,
                    Record<
                      string,
                      { pedidos: number; carcaçasDevolvidas: number }
                    >
                  > = {};
                  ...
                  orders.forEach((pedido) => {
                    ...
                    if (
                      ...
                      pedido.vendedor_id !== vendedorFiltro
                    )
                      return;

                    const dataVenda = new Date(pedido.data_venda);
                    const mesAno = `${dataVenda.getFullYear()}-${String(
                    ...
                    if (!desempenhoMensal[mesAno][vendedorId]) {
                      desempenhoMensal[mesAno][vendedorId] = {
                        pedidos: 0,
                        carcaçasDevolvidas: 0,
                      };
                    }
                  ...
                  const mesesOrdenados = Object.keys(desempenhoMensal)
                    .sort()
                    .reverse();
          ...
          <TabsContent value="vendedores" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Análise de vendas e débitos por vendedor
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {vendedoresComDados
                      .filter(
                        (v) =>
                          vendedorFiltro === "todos" || v.id === vendedorFiltro
                      )
                      ...
                      .map((vendedor) => (
                        <TableRow key={vendedor.id}>
                          ...
                          <TableCell className="text-right">
                            {formatCurrency(vendedor.valorVendas)}
                          </TableCell>
                          <TableCell className="text-right">
                            {vendedor.carcacasPendentesAtual}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatCurrency(vendedor.debitoAtual)}
                          </TableCell>
                        </TableRow>
                      ))}
                  </TableBody>
          ...
          <TabsContent value="clientes" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Histórico de compras e débitos por cliente
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                      ...
                      .map((cliente) => {
                        ...
                        return (
                          <TableRow key={cliente.id}>
                            ...
                            <TableCell className="text-right">
                              {formatCurrency(cliente.valorVendas)}
                            </TableCell>
                            <TableCell className="text-right">
                              {cliente.carcacasPendentesAtual}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatCurrency(cliente.debitoAtual)}
                            </TableCell>
                          </TableRow>
                        );
                      })}
          ...
          <TabsContent value="carcacas" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Todas as carcaças aguardando devolução no período
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                    ...
                    {carcacasPendentes.length === 0 ? (
                      ...
                      (() => {
                        ...
                        carcacasPendentes
                          .sort(
                            (a, b) =>
                              getDaysPending(b.data_venda) -
                              getDaysPending(a.data_venda)
                          )
                          .forEach((pedido) => {
                        ...
                        return itensPendentes.map(
                          ({ pedido, item, cliente, vendedor }, idx) => (
                            <TableRow key={`${pedido.id}-${item.id}-${idx}`}>
                              ...
                              <TableCell className="text-right">
                                {item.debito_carcaca || 0} carcaça(s)
                              </TableCell>
                              <TableCell className="text-center">
                                {getDaysPending(pedido.data_venda)}
                              </TableCell>
                              <TableCell>
                                <StatusBadge
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Exportar relatório completo de vendas do período
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
                    onClick={() => handleExportarPDF("vendas")}
                    className="w-full"
                  >
                    <Download className="mr-2 h-4 w-4" />
                    Exportar Vendas
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Exportar relatório de carcaças pendentes
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
                    onClick={() => handleExportarPDF("carcacas")}
                    className="w-full"
                  >
                    <Download className="mr-2 h-4 w-4" />
                    Exportar Carcaças
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Exportar relatório de pedidos pendentes
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Exportar histórico de clientes
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
