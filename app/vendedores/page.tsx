...
export default function VendedoresPage() {
  ...
  const loadVendedores = async () => {
    try {
      setIsLoading(true);
      const [vendedoresData, ordersData] = await Promise.all([
        getVendedores(),
        getOrders(),
      ]);
  ...
  const vendedoresComDados = vendedores.map((vendedor) => {
    ...
    const vendasBaseTroca = pedidosVendedor.filter(
      (p) => p.tipo_venda === "Base de Troca"
    ).length;
    const percentualBaseTroca =
      totalVendas > 0 ? (vendasBaseTroca / totalVendas) * 100 : 0;
  ...
  const debitoTotal = vendedoresComDados.reduce(
    (acc, v) => acc + v.debitoTotal,
    0
  );
  const carcacasTotal = vendedoresComDados.reduce(
    (acc, v) => acc + v.carcacasPendentes,
    0
  );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Gerencie e visualize o desempenho dos vendedores
            </p>
          </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-muted-foreground">
                              {vendedor.vendasBaseTroca} de{" "}
                              {vendedor.totalVendas}
                            </span>
                            <span className="font-medium">
                              {vendedor.percentualBaseTroca.toFixed(0)}%
                            </span>
                          </div>
                          <Progress
                            value={vendedor.percentualBaseTroca}
                            className="h-1"
                          />
                        </div>
                      </TableCell>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Preencha os dados do novo vendedor do sistema
            </DialogDescription>
          </DialogHeader>
